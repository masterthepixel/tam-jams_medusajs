...
colors:
  # Action Blue — Apple's real primary CTA/link color.
  primary: "#0071e3"
  # Darker press/active tint (formerly the primary value).
  primary-press: "#0066cc"
  # Brighter sibling used on dark surfaces for links + dark-surface focus ring.
  ...
  body-muted: "#cccccc"
  ink-muted-80: "#333333"
  # AA-large / decorative only — fails WCAG AA for normal text on parchment.
  ink-muted-48: "#7a7a7a"
  divider-soft: "#f0f0f0"
  hairline: "#e0e0e0"
  canvas: "#ffffff"
...
spacing:
  ...
  xs: 8px
  sm: 12px
  # md (17px) is intentionally off the 8px grid — it tracks the 17px body type rhythm.
  md: 17px
  lg: 24px
  xl: 32px
  xxl: 48px
...
components:
  ...
  button-primary-focus:
    ...
    rounded: "{rounded.pill}"
  button-primary-focus-on-dark:
    backgroundColor: "{colors.primary}"
    textColor: "{colors.on-primary}"
    outline: "2px solid {colors.primary-on-dark}"
    rounded: "{rounded.pill}"
...
### Brand & Accent
- **Action Blue** (`{colors.primary}` — #0071e3): The single brand-level interactive color. All text links, all blue pill CTAs ("Learn more", "Buy"), and the focus ring root. This is Apple's quiet but universal "click me" signal.
- **Press Blue** (`{colors.primary-press}` — #0066cc): A darker sibling used as the pressed/active tint where a hex shift is needed in addition to the system-wide `scale(0.95)` transform.
- **Sky Link Blue** (`{colors.primary-on-dark}` — #2997ff): A brighter blue used on dark surfaces for in-copy links, inline callouts, and the focus ring on dark tiles, where Action Blue would disappear against the tile background.
...
- **Near-Black Tile 2** (`{colors.surface-tile-2}` — #2a2a2c): A micro-step lighter — used where a dark tile sits directly above or below Tile 1 to create the faintest separation. Sub-perceptual; structural only.
- **Near-Black Tile 3** (`{colors.surface-tile-3}` — #252527): A micro-step darker — used at the bottom of the stack and in embedded video/player frames. Sub-perceptual; structural only.
...
- **Ink Muted 48** (`{colors.ink-muted-48}` — #7a7a7a): Disabled button text and legal fine-print. **Accessibility note:** ~3.5:1 on parchment — AA-large / decorative only, not for normal-size body text.

### Hairlines & Borders
- **Divider Soft** (`{colors.divider-soft}` — #f0f0f0): The "border" tone on secondary buttons and the search input — functions as a ring shadow rather than a hard line. In production, often applied as `rgba(0, 0, 0, 0.04–0.08)`.
...
| `{typography.display-md}` | 34px | 600 | 1.18 | -0.374px | Section heads (SF Pro Text at display proportions) |
| `{typography.lead}` | 28px | 400 | 1.14 | 0.196px | Product tile subcopy |
| `{typography.lead-airy}` | 24px | 300 | 1.5 | 0 | Environment-page lead paragraphs (the rare weight 300) |
| `{typography.tagline}` | 21px | 700 | 1.19 | 0.231px | Sub-tile tagline; sub-nav category name (the rare weight 700) |
| `{typography.body-strong}` | 17px | 600 | 1.24 | -0.374px | Inline strong emphasis |
| `{typography.body}` | 17px | 400 | 1.47 | -0.374px | Default paragraph |
| `{typography.button-pill}` | 17px | 400 | 1.0 | -0.374px | Pill CTA labels (single-line inside the capsule) |
| `{typography.dense-link}` | 17px | 400 | 2.41 | 0 | Footer / store utility link lists (relaxed leading) |
| `{typography.caption}` | 14px | 400 | 1.43 | -0.224px | Secondary captions, button text |
| `{typography.caption-strong}` | 14px | 600 | 1.29 | -0.224px | Emphasized captions |
| `{typography.button-large}` | 18px | 300 | 1.0 | 0 | Store hero CTAs (the rare weight 300) |
| `{typography.button-utility}` | 14px | 400 | 1.29 | -0.224px | Utility/nav button labels |
| `{typography.fine-print}` | 12px | 400 | 1.2 | -0.12px | Fine-print, footer body |
| `{typography.micro-legal}` | 10px | 400 | 1.3 | -0.08px | Micro legal disclaimers |
| `{typography.nav-link}` | 12px | 400 | 1.2 | -0.12px | Global nav menu items |
...
- **Line-height is context-specific.** Display sizes use 1.07–1.19 (tight). Body uses 1.47. Utility link stacks in the footer/store use an unusually relaxed 2.41 (`{typography.dense-link}`) — intentionally off the 8px grid; it's how the footer's dense link columns breathe.
...
- **Note:** `{spacing.md}` (17px) is intentionally off the 8px grid — it mirrors the 17px body type rhythm and should not be "corrected" to 16px.
...
| `{rounded.full}` | 9999px (or `50%`) | Circular control chips floating over photography |
...
**`sub-nav-frosted`** — Surface-specific nav that sticks below the global nav. Background `{colors.canvas-parchment}` at 80% opacity with backdrop-filter blur, creating a frosted-glass effect. Height 52px. Content on left: product category name ("iPhone", "Store", "Accessories") in `{typography.tagline}` (21px / 700). Content right: inline nav links in `{typography.button-utility}` (14px), ending in a persistent `{component.button-primary}` ("Buy") or a utility link.

### Buttons

**`button-primary`** — The signature Apple action. Background `{colors.primary}` (Action Blue #0071e3), text `{colors.on-primary}` in `{typography.button-pill}` (SF Pro Text 17px / 400 / single-line), rounded `{rounded.pill}` (full pill — capsule-shaped), padding 11px × 22px. The full-pill radius IS the brand action signal.
- Active state: `{component.button-primary-active}` — `transform: scale(0.95)` (the system-wide micro-interaction).
- Focus state (light): `{component.button-primary-focus}` — 2px solid `{colors.primary}` outline.
- Focus state (dark tile): `{component.button-primary-focus-on-dark}` — 2px solid `{colors.primary-on-dark}` so the ring remains visible against near-black.
...
**`text-link`** — Inline body links in `{colors.primary}` (Action Blue). Resting state has no underline; underline appears on interaction only.
...
**`configurator-option-chip`** — Pill-shaped tappable cell used in the iPhone 17 Pro buy page. Background `{colors.canvas}`, 1px solid `{colors.hairline}` border, text `{colors.ink}` in `{typography.caption}`, rounded `{rounded.pill}`, padding 12px × 16px. Contains a small product thumbnail + label + price delta. Arranged in a grid of 4–5 options per row.
...
**`search-input`** — The accessories search input. Background `{colors.canvas}`, text `{colors.ink}` in `{typography.body}` (17px), 1px solid `{colors.divider-soft}` border (`rgba(0, 0, 0, 0.08)` in production), rounded `{rounded.pill}` (full pill — search is also pill-shaped, matching the CTA grammar), padding 12px × 20px, height 44px. Leading icon: search glyph at 14px, muted tint.
...
## Changelog (local corrections)

This local copy applies the following fixes versus the upstream `getdesign add apple` spec:

1. **`primary` → #0071e3** (Apple's real Action Blue). The former `#0066cc` is preserved as `primary-press` for the pressed/active tint.
2. **`tagline` weight 600 → 700**, reconciling the token with the prose "rare weight 700" exception.
3. **`display-md` line-height 1.47 → 1.18**, aligning the 34px section head with the other display tokens (was body leading).
4. **Borders promoted from prose to explicit token fields** on `button-secondary-pill`, `button-pearl-capsule`, `store-utility-card`, `configurator-option-chip`/`-selected`, and `search-input`. Also fixed `button-secondary-pill` background `canvas → transparent` to match its "ghost pill" description.
5. **New `button-pill` type token** (17px / 400 / line-height 1.0) so pill CTAs no longer inherit body's 1.47 leading; `button-primary`/`-secondary-pill` now reference it.
6. **Dark-surface focus variant** `button-primary-focus-on-dark` (2px `primary-on-dark` ring) for visibility on near-black tiles.
7. **Minor a11y/leading nudges**: `fine-print`/`nav-link` line-height 1.0 → 1.2 to avoid descender clipping; `ink-muted-48` flagged AA-large/decorative only.
